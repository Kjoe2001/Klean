      ...
      <div className="w-full max-w-[420px]">
        ...
        <div className="card p-8">
          ...
          <p className="text-center text-sm text-[#6B7280] mb-7">
            {mode === 'signup' ? 'Free 7-day trial · 30 credits · No card required' : 'Log in to your Zelvoo dashboard'}
          </p>

          <div className="flex flex-col gap-2.5 mb-6">
            <button onClick={() => oauth('google')} className="btn-outline w-full flex items-center justify-center gap-2">
            ...
            <button onClick={() => oauth('azure')} className="btn-outline w-full flex items-center justify-center gap-2">
          ...
          <p className="text-center text-xs text-[#6B7280] mt-5">
            {mode === 'signup'
              ? <>Already have an account? <Link className="text-primary font-semibold hover:underline" href={plan ? `/login?plan=${plan}` : '/login'}>Log in</Link></>
              : <><Link className="text-primary font-semibold hover:underline" href="/forgot-password">Forgot password?</Link> · <Link className="text-primary font-semibold hover:underline" href="/signup">Start free trial</Link></>}
          </p>
        </div>
      </div>
    </div>
