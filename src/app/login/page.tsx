...
function Form({ mode }: { mode: 'signup' | 'login' }) {
  ...
  useEffect(() => {
    ...
    if (oauthError === 'missing_oauth_code') {
      setErr('Sign-in did not complete. Please try Google login again.');
      return;
    }
    if (oauthError === 'oauth_exchange_failed') {
      setErr('Google sign-in failed to create a session. Try again or use email/password.');
      return;
    }
      ...
      <div className="w-full max-w-[460px] relative z-10">
        ...
        <div className="glass-elevated glass-highlight p-8 md:p-9">
          ...
          <div className="flex flex-col gap-2.5 mb-6">
            {ENABLE_GOOGLE_OAUTH && <button onClick={() => {}}
            ...
            {ENABLE_AZURE_OAUTH && <button onClick={() => {}}
              className="btn-outline w-full flex items-center justify-center gap-2">
